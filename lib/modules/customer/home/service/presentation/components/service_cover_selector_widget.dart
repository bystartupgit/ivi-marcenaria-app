...
class ServiceCoverSelectorWidget extends StatelessWidget {
  ...
  final String title = "Deseja facilitar ainda mais a encontra seu projeto em nosso app? ";
                    ...
                    text: TextSpan(
                      children: <TextSpan>[
                        TextSpan(text: title, style: TextStyle(fontSize: 18,
                            color: ColorTheme.title, fontFamily: FamilyTheme.bold)),
                        TextSpan(text: subtitle, style: TextStyle(fontSize: 12,
                            color: ColorTheme.title, fontFamily: FamilyTheme.regular)),
                      ])),
                    ...
                    child: Row(
                      ...
                      children: [
                        Row(
                            ...
                            children: ServiceCoverMapper.covers.map((e) =>
                                ServiceCoverItemWidget(image: e, selected: store.cover.$1 == e,
                                    onPressed: () => store.setCover((e,CoverType.asset)))).toList()),
                        Row(
                            spacing: 5.0,
                            mainAxisSize: MainAxisSize.min,
                            children: store.coverFiles.map((e) =>
                                ServiceCoverFileItemWidget(image: e, selected: store.cover.$1 == e.path,
                                    onPressed: () => store.setCover((e.path,CoverType.file)))).toList()),

                        ServiceCoverButtonWidget(onPressed: () => store.addCoverFile(context: context))
                      ]))
                ])
          ));
      });
