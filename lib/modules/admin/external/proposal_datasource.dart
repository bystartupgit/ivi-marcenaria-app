...
class ProposalDataSource {
  ...
  Future<(String, bool)> createProposal(
      {required CreateProposalDTO dto}) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        return ("Sucesso ao criar proposta", true);
      } else {
        return (
          "Não foi possível criar a proposta. Tente novamente mais tarde",
          false
        );
      }
    } catch (e) {
  ...
  Future<(String, bool)> selectionEmployeeToExecution(
      {required int proposalID, required int employeeID}) async {
    Uri url =
        Uri.parse("$enviroment/api/propostas/$proposalID/selecionar-execucao");
    ...
    try {
      ...
      if (response.statusCode == 201) {
        return ("Sucesso ao selecionar prestador. ", true);
      } else {
        return (
          "Não foi possível selecionar o prestador. Tente novamente mais tarde",
          false
        );
      }
    } catch (e) {
