        ...
        child: Column(
          children: [
            ProposalFilterWidget(
                onChanged: (value) =>
                    store.setFilter(value, context, widget.proposalID)),
            const SizedBox(height: 20),
            store.employees.isEmpty
                ? Expanded(
                    child: Center(
                        child: Text(
                      "Nenhum prestador demonstrou interesse na proposta no momento.",
                        ...
                        itemBuilder: (context, index) =>
                            Observer(builder: (context) {
                              return EmployeeCardWidget(
                                  selected: store.employeedSelected ==
                                      store.employees[index],
                                  select: store.employees[index]
                                              .statusInteresse ==
                                          "interessado"
                                      ? store.employeedSelected ==
                                              store.employees[index]
                                          ? () => store.setSelectEmployee(null)
                                          : () => store.setSelectEmployee(
                                              store.employees[index])
                                      : null,
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                      ...
                      children: [
                        const Expanded(
                            flex: 1, child: CreateProporseBackButtonWidget()),
                        Expanded(
                            flex: 2,
                            child: ConfirmationButtonWidget(
                                onPressed: store.hasSelected
                                    ? () => store.save(
                                        context: context,
