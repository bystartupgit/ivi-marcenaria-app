...
class UserDataSource {
  ...
  Future<EmployeeEntity?> getUserDetails(
  ...
  Future<bool> updateProfile(
      {required int userID, required ProfileDTO dto}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        return data[ProfileMappers.message] ==
            "Dados do prestador atualizados com sucesso";
      } else {
        return false;
      }
    } catch (e) {
  ...
  Future<bool> registerMedia(
    ...
    try {
      ...
      if (response.statusCode == 201) {
        Modular.get<CoreStore>().setPathImage(path);
        return true;
      } else {
        return false;
      }
  ...
  Future<bool> registerDocumentMedia(
      {required File photo,
      required int employeeID,
