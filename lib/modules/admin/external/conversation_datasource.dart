...
class ConversationDataSource {
  ...
  Future<List<ConversationOrderEntity>> getOrderConversations(
    ...
    if (response.statusCode == 200) {
      ...
      if (orders.isNotEmpty) {
        List<ConversationOrderEntity> conversations =
            orders.map((e) => ConversationOrderEntity.fromMap(e)).toList();
  ...
  Future<void> getprivateConversations(
      {required int page, required int limit, String title = ""}) async {
    Uri url = Uri.parse(
        "$enviroment/api/mensagens/lista-usuarios-com-mensagem-suporte");
