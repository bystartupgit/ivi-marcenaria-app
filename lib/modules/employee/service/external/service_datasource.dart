...
class ServiceDataSource {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (result.isNotEmpty) {
          return result
              .map((e) => OrderEntity.fromMap(
                  e[OrderMapper.proposal][OrderMapper.order]))
              .toList();
        } else {
          return [];
        }
  ...
  Future<List<OrderEntity>> getServiceFinish(
      {required int employeeID, required int page, required int limit}) async {
    String? token = Modular.get<CoreStore>().auth?.token;

    Uri url =
        Uri.parse("$enviroment/api/pedidos/prestador/$employeeID/concluidos");
  ...
  Future<(String, bool)> finishService({required int proposalID}) async {
    ...
    Uri url =
        Uri.parse("$enviroment/api/propostas/$proposalID/conclusao-execucao");
