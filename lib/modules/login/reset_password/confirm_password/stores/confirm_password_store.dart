...
class ConfirmPasswordStore = ConfirmPasswordStoreBase
    with _$ConfirmPasswordStore;
    ...
    try {
      if (passwordValid == false) {
        ShowErrorMessageUsecase(context: context)
            .call(message: "Senha e Confirmar Senha não sáo válidas.");
      }

      setLoading(true);

      (String, bool) result =
          await _resetPasswordUsecase.call(code: code, password: password);

      if (result.$2) {
        ShowSuccessMessageUsecase(context: context)
            .call(message: "Sucesso ao redefinir senha")
            .whenComplete(() =>
