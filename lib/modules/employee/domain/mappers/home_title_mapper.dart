

class HomeTitleMapper {


  static String oportunities = "Oportunidades";
  static String pendentes = "Serviços pendentes";
  static String progress = "Serviços em andamento";

  static String notification = "Notificações";
  static String home = "Inicio";
  static String orcamentos = "Orçamentos";
  static String orders = "Pedidos";
  static String profile = "Perfil";
  static String messages = "Mensagens";

  static String exit = "Sair";

}