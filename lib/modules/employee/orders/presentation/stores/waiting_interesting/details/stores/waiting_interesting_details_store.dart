...
class WaitingInterestingDetailsStore = WaitingInterestingDetailsStoreBase
    with _$WaitingInterestingDetailsStore;

abstract class WaitingInterestingDetailsStoreBase with Store {
  final AcceptProposalUsecase _acceptProposalUsecase =
      Modular.get<AcceptProposalUsecase>();
  ...
  approvalProposal({required context}) async {
    try {
      ...
      if (result) {
        Modular.to.pushNamed(RouterMapper.orderWaitingSuccessDetailsIntern);
      } else {
        ShowErrorMessageUsecase(context: context).call(
            message:
                "Não foi possível aceitar a proposta. Tente novamente mais tarde");
      }
    } catch (e) {
      ShowErrorMessageUsecase(context: context)
          .call(message: "Erro ao tentar aceitar a proposta.");
    } finally {
      setLoading(false);
    }
  }
