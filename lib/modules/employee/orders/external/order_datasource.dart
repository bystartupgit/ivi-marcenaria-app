...
class OrderDataSource {
  ...
  Future<List<OrderEntity>> getWaitingInterestOrders(
      {required int employeeID, required int page, required int limit}) async {
    String? token = Modular.get<CoreStore>().auth?.token;

    Uri url = Uri.parse(
        "$enviroment/api/pedidos/prestador/$employeeID/selecionado-aceitar");
    ...
    if (response.statusCode == 200) {
      ...
      if (result.isEmpty) {
        ...
        return result
            .map((e) =>
                OrderEntity.fromMap(e[OrderMapper.proposal][OrderMapper.order]))
            .toList();
      }
    } else {
      return [];
  ...
  Future<List<OrderEntity>> getOrderNotStarted(
      {required int employeeID, required int page, required int limit}) async {
    String? token = Modular.get<CoreStore>().auth?.token;

    Uri url =
        Uri.parse("$enviroment/api/pedidos/prestador/$employeeID/sem-execucao");
  ...
  Future<(OrderEntity?, ProposalEntity?)> getOrderDetails(
  ...
  Future<bool> registerInteresting(
  ...
  Future<bool> startService({required int proposalID}) async {
    Uri url =
        Uri.parse("$enviroment/api/propostas/$proposalID/iniciar-execucao");
