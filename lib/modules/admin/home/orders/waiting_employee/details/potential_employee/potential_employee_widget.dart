...
class PotentialEmployeeWidget extends StatefulWidget {
  ...
  @override
  State<PotentialEmployeeWidget> createState() => _PotentialEmployeeWidgetState();
}
    ...
    return Observer(
        builder: (context) {
           return Container(
            ...
            child: store.loading? Center(child: CircularProgressIndicator(color: ColorTheme.orange)) : Column(
              children: [
                ProposalFilterWidget(onChanged: (value) => store.setFilter(value, context)),
                      ...
                      itemBuilder: (context,index) => Observer(
                        builder: (context) {
                          return EmployeeCardWidget(
                              selected: store.employeeSelected.contains(store.employees[index]) ,
                              select: store.employeeSelected.contains(store.employees[index]) ?
                                  () => store.removeSelection(store.employees[index]) :
                                  () =>store.addSelection(store.employees[index]),
                    ...
                    Padding(
                      ...
                      child: Row(
                          ...
                          children: [
                            ...
                            Expanded(
                                flex: 2,
                                child: ConfirmationButtonWidget(onPressed: store.hasEmployeeSelected ?
                                    () => store.save(context: context, proposalID: widget.proposalID) : null, title: 'Designar prestador', size: 12))
                          ]),
                    ),
