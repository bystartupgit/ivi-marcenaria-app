...
class ProposalPaymentCardWidget extends StatefulWidget {
  ...
  @override
  State<ProposalPaymentCardWidget> createState() => _ProposalPaymentCardWidgetState();
}
...
class _ProposalPaymentCardWidgetState extends State<ProposalPaymentCardWidget> {
  ...
  Widget build(BuildContext context) {
    return moreDetails ? ProposalIncompleteCardWidget(showMore: () => changeMoreDetails()) : Container(
        ...
        child: Column(
            ...
            spacing: 5.0,
            children: [
              Text("Entrada + Parcelado: ${"R\$ ${CoinUtil.formatValues(double.parse(proposal.entradaMinima))}"} + ${proposal.parcelas}x ${"R\$ ${CoinUtil.formatValues(double.parse(proposal.valorParc ?? "0"))}"}",
                  style: TextStyle(color: ColorTheme.black3, fontSize: 10, fontFamily: FamilyTheme.regular )),
              Text(DateUtil.convertDateTimeToStringBrazilFormt(proposal.dataCriacao),style: TextStyle(color: ColorTheme.black3, fontSize: 10, fontFamily: FamilyTheme.regular)),
              Row(
