...
class OrderPedingDetailsStore = OrderPedingDetailsStoreBase with _$OrderPedingDetailsStore;

abstract class OrderPedingDetailsStoreBase with Store {

  final StartServiceUsecase _startServiceUsecase = Modular.get<StartServiceUsecase>();
  ...
  startService({required context}) async {
    ...
    try {
      ...
      bool result = await _startServiceUsecase.call(proposalID: proposal?.idProposta ?? 0);

      if(result) { Modular.to.pop(true); }
      else { ShowErrorMessageUsecase(context: context).call(message: "Não foi possível iniciar o serviço. Tente novamente mais tarde"); }

    } catch(e) { ShowErrorMessageUsecase(context: context).call(message: "Erro ao tentar iniciar o serviço.");  } finally { setLoading(false); }
  }
