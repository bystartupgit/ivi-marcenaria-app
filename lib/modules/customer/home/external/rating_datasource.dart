...
class RatingDatasource {
  ...
  Future<(String, bool)> registerRating({required RatingAPPEntity app, required RatingEmployeeEntity employee}) async {
    ...
    await post(
        url, headers: headers, body: jsonEncode(bodyAPP))
        .timeout(const Duration(seconds: 8)).then((e) => print(e.body));



    Response response =  await post(
        url, headers: headers, body: jsonEncode(bodyEmployee))
    ...
    if(response.statusCode == 201) { return (message,true); }
